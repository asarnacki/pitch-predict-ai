...
export function MatchCard({ match, predictionState, onSavePrediction }: MatchCardProps) {
  ...
  return (
    <AccordionItem value={match.id} className="border rounded-lg overflow-hidden bg-card">
        ...
        <div className="flex flex-col gap-3 w-full pr-2 sm:pr-4">
          ...
          <div className="flex items-center justify-center gap-3 sm:gap-4">
            <div className="flex items-center gap-2 flex-1 justify-end">
              <span className="font-semibold text-sm sm:text-base text-right truncate">{match.home_team.name}</span>
              <img
            ...
            <div className="flex items-center gap-2 flex-1 justify-start">
              <img
                ...
                className="w-6 h-6 sm:w-8 sm:h-8 flex-shrink-0"
              />
              <span className="font-semibold text-sm sm:text-base text-left truncate">{match.away_team.name}</span>
            </div>
          </div>

          <div className="text-xs sm:text-sm text-muted-foreground text-center">{match.league}</div>
        </div>
      </AccordionTrigger>
