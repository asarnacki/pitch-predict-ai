...
const ListRoot = forwardRef<HTMLDivElement, HIGListProps>(({ children, className, inset, ...props }, ref) => (
  <div ref={ref} className={cn("flex flex-col gap-6", inset && "px-4", className)} {...props}>
    {children}
  </div>
));
...
const ListSection = ({ children, className, footer, header, ...props }: HIGListSectionProps) => (
  <section className={cn("space-y-2", className)} {...props}>
    {header ? (
      <div className="px-4 text-[0.85rem] uppercase tracking-[0.18em] text-[color:var(--hig-color-label-tertiary)]">
        {header}
      </div>
    ) : null}
  ...
  return (
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        {label ? (
          <span className="truncate text-[1rem] font-medium text-[color:var(--hig-color-label-primary)]">{label}</span>
        ) : null}
        {subtitle ? (
          <span className="truncate text-[0.9rem] text-[color:var(--hig-color-label-secondary)]">{subtitle}</span>
        ) : null}
        {context ? (
          <span className="truncate text-[0.78rem] text-[color:var(--hig-color-label-tertiary)]">{context}</span>
        ) : null}
        {children}
      </div>
      {detail ? (
      ...
      {accessory ? <div className="ml-2 text-[color:var(--hig-color-label-tertiary)]">{accessory}</div> : null}
    </Component>
  );
};
