...
export const GET: APIRoute = async ({ locals, params }) => {
  try {
    ...
    const prediction = await getPredictionById(locals.supabase, locals.user.id, validatedParams.id);
...
export const DELETE: APIRoute = async ({ locals, params }) => {
  try {
    ...
    const deleted = await deletePrediction(locals.supabase, locals.user.id, validatedParams.id);
